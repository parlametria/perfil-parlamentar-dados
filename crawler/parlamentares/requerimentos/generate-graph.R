...
generate_graph <- function(propositions_filepath = here::here("crawler/raw_data/autores_requerimentos_leggo.csv"), 
                           output_file) {
  
  df <- read.csv(propositions_filepath)
  
  pre_nodes <- df %>% 
    dplyr::select(nome, id_req) %>% 
    dplyr::distinct(nome) %>% 
