...
composicao_comissoes <- composicao_comissoes %>% 
  right_join(comissoes, by=c("comissao_id" = "id")) %>% 
  filter(situacao == "Titular") %>% 
  select(id_parlamentar, sigla)

aderencia <- 
  processa_aderencia_parlamentares(
    votos_path = here::here("reports/aderencia-senado/data/votos.csv"),
    orientacoes_path = here::here("reports/aderencia-senado/data/orientacoes.csv"),
...
data_long <- aderencia_governo %>% 
  arrange(desc(seguiu), freq, faltou) %>% 
  head(20) %>% 
  gather("match", "n", faltou:seguiu) %>% 
  filter(match %in% match_valores)
