...
deputados_receita %>% 
  ...
  scale_shape_identity() +
  geom_point(aes(x = median, color = "a"), size = 3.7, shape = 124) +
  scale_x_continuous(breaks = seq(0, 100, 5)) +
  ...
  scale_color_manual(
    name = "", values = c("#dc267f"), guide = FALSE
  ) +
  theme_ipsum_rc() 
```
...
senadores_receita %>% 
  ...
  geom_point(aes(y = proporcao_receita_lider*100), color = "#dc267f", size = 1) +
  scale_x_discrete(position = "right") +
  scale_y_continuous(limits = c(0, 45), breaks = seq(0, 100, 5), position = "bottom", sec.axis = dup_axis()) +
  scale_color_manual(values = rep(c("#648fff", "#ffb000"), 15)) +
  coord_flip() +
  scale_shape_identity() +
  
  annotate("text", label = "Mediana do partido", x = "MDB", y = 14, color = "#4b4545") +
  geom_curve(aes(x = 10.25, y = 7.55, xend = "MDB", yend = 10),
             color = "#7d7373", size = 0.4, curvature = -0.5,
             arrow = arrow(length = unit(0.3, "cm"), ends="first")) +
  
  annotate("text", label = "líder do partido", x = 10.4, y = 31.8, color = "#4b4545") +
  geom_curve(aes(x = 10.25, y = 38, xend = 10.4, yend = 35),
             color = "#7d7373", size = 0.4, curvature = 0.5,
             arrow = arrow(length = unit(0.3, "cm"), ends="first")) +
...
senadores_receita %>% 
  ...
  annotate("text", label = "Mediana", x = 14.8, y = 1.72, color = "#4b4545") +
  geom_curve(aes(x = 9.9, y = 1.9, xend = 12.5, yend = 1.7),
             color = "#7d7373", size = 0.4, curvature = 0.35,
             arrow = arrow(length = unit(0.3, "cm"), ends="first")) +

  annotate("text", label = "um deputado", x = 44.3, y = 1.62, color = "#4b4545") +
  geom_curve(aes(x = 38.55, y = 1.9, xend = 41, yend = 1.6),
             color = "#7d7373", size = 0.4, curvature = 0.38,
             arrow = arrow(length = unit(0.3, "cm"), ends="first")) +

  scale_shape_identity() +
  geom_point(aes(x = median, color = "a"), size = 1, shape = 124, stroke = 4) +
  scale_x_continuous(breaks = seq(0, 100, 5)) +
  labs(y = "Nº de mandatos", x = "Doado pelo Partido (%)") +
  scale_fill_manual(name = "", values = c("#91BFDA"), guide = FALSE) +
  scale_color_manual(name = "", values = c("#dc267f"), guide = FALSE) +
  theme_ipsum_rc() +
...
ggplot(
  ...
  geom_text(size = 3.5, fontface = "bold", nudge_y = 0.6,
            mapping = aes(label = ifelse(sg_partido == "PSDB", 
                                         ifelse(group == "F",
                                               "Mulher",
                                               "Homem"),
                                          ""),
  ...
  ## retangulo das diferencas
  geom_rect(mapping = aes(xmin = 26.3, xmax = Inf , ymin = -Inf, ymax = Inf),
            fill = "white",
            color = "white") +
  geom_rect(mapping = aes(xmin = 26.4, xmax = 29 , ymin = -Inf, ymax = Inf),
            fill = "#eef0e2",
            color = "#eef0e2") +
  geom_text(fontface = "bold", size = 3.3, colour = "#5f5757", 
            mapping = aes(x = 27.7, y = sg_partido, 
                          label = ifelse(group == "M", "",
                                         ifelse(diff > 0, paste0("+", as.character(round(diff, 2))), 
                                                paste0(as.character(round(diff, 2)))
                                                )
  ...
  geom_text(size = 3.1, colour = "#9e9191", nudge_y = 0.6,
            mapping = aes(x = 27.7, y = sg_partido, label = ifelse(sg_partido == "PSDB", "diferença", ""))) +
  ## fim retangulo das diferencas
  
  scale_color_manual(values = c("#DEAC7C", "#83ACB8")) +
  scale_x_continuous(breaks = seq(0, 26, 1), limits = c(0, 29)) +
  scale_y_discrete(
    expand = expand_scale(add=c(0.65,1))
  ) +
  labs(title = "Proporção mediana do investimento \npor gênero e partido",
