...
```{r}
receita_raw <- read_csv(here("crawler/raw_data/receitas_tse_2018.csv"))
...
A seguir iremos realizar a análise considerando dados de Senadores. A diferença para análise dos deputados, em que consideramos a média dos deputados na UF, é que para o Senado, consideramos a média do valor investido em cargos do congresso. Ou seja, considerando a soma do valor investido nos deputados e nos senadores na média da UF. Coletamos dados de `r senadores_receita %>% filter(!is.na(total_receita)) %>% nrow()` senadores dos 81 atuais. Nas eleições de 2018, 2 senadores de cada unidade federativa foram eleitos para um mandato de 8 anos.
