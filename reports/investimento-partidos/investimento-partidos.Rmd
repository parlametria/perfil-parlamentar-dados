...
deputados_receita <- deputados %>% 
  left_join(receita %>% 
              select(cpf, partido, total_receita, proporcao_receita, n_candidatos), 
...
deputados_receita %>% 
  ...
  # geom_curve(aes(x = "NOVO", y = 5.3, xend = "PT", yend = 4.5),
  ...
  theme_ipsum_rc() +
  geom_text(aes(label = ifelse(proporcao_receita > 100, 
...
ggplot(
  ...
  geom_text(size = 3.5, fontface = "bold", nudge_y = 0.6,
            mapping = aes(label = ifelse(sg_partido == "DEM", 
                                         ifelse(group == "F",
                                               "Mulher",
                                               "Homem"),
                                          ""),
  ...
  geom_rect(mapping = aes(xmin = 103, xmax = Inf , ymin = -Inf, ymax = Inf),
            fill = "white",
            color = "white") +
  geom_rect(mapping = aes(xmin = 103.5, xmax = 115.5 , ymin = -Inf, ymax = Inf),
            fill = "#eef0e2",
            color = "#eef0e2") +
  geom_text(fontface = "bold", size = 3.3, colour = "#5f5757",
            mapping = aes(x = 109.5, y = sg_partido,
  ...
  geom_text(size = 3, colour = "#9e9191", nudge_y = 0.6,
            mapping = aes(x = 109.5, y = sg_partido, label = ifelse(sg_partido == "DEM", "diferença", ""))) +
  ## fim retangulo das diferencas
  
  scale_color_manual(values = c("#DEAC7C", "#83ACB8")) +
  scale_x_continuous(breaks = seq(0, 100, 10), limits = c(0, 120)) +
  scale_y_discrete(
...
senadores_receita <- senadores %>%
  ...
  select(id, casa, nome_eleitoral, uf = uf.x, genero, sg_partido, total_receita, 
         total_partido, proporcao_receita, n_candidatos) %>% 
  mutate(nome = paste0(str_to_title(nome_eleitoral), " - ", sg_partido, "/", uf))
```
...
senadores_receita %>% 
  ...
  geom_chicklet(width = .7, radius = grid::unit(5, "pt")) +
  coord_flip() +
  geom_text(aes(label = n_candidatos), nudge_y = ) +
  scale_fill_manual(values = c("#806CCC")) +
  scale_y_continuous(breaks = seq(0, 100, 5)) +
  guides(fill = FALSE) + 
  labs(title = "Senadores com maior investimento do Partido",
...
senadores_receita %>% 
  ...
  geom_point(aes(y = proporcao_receita_lider*100), color = "#dc267f", size = 1) +
  scale_x_discrete(position = "right") +
  scale_y_continuous(limits = c(0, 140), breaks = seq(0, 140, 5), position = "bottom", sec.axis = dup_axis()) +
  scale_color_manual(values = rep(c("#648fff", "#ffb000"), 15)) +
  coord_flip() +
  scale_shape_identity() +
  ...
  guides(color = FALSE, size = FALSE) +
  theme_ipsum_rc() +
  geom_text(aes(label = ifelse(proporcao_receita > 85, 
                               paste0(str_to_title(nome_eleitoral), " ", sg_partido, "/", uf), 
                               "")),
            hjust = -.07,
            size = 3.5,
