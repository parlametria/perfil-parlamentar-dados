...
autores <- get_dataset_autores(here::here("data/autores.csv"))

parlamentares <- get_dataset_parlamentares(here::here("data/parlamentares.csv"))

proposicoes <- get_dataset_proposicoes(here::here("data/tabela_leggo.csv"))
...
output$hist_nodes <- renderPlot({
    getCoautorias()[[1]] %>%
    ggplot(aes(x = group)) +
    geom_histogram(boundary = 0, fill = "#1BB5A9", color = "#35193A", stat="count") +
    labs(x = "Peso das relações", y = "Número de relações") +
    theme_ipsum_rc()
