...
fetch_orientacoes_camara <- function(id_proposicao, id_votacao, resumo_votacao, objeto_votacao) {
  library(xml2)
  ...
  orientacoes <- xml2::xml_find_all(xml, paste0(".//Votacao[@ObjVotacao = '",
                                                votacao$obj_votacao, "']",
  ...
  orientacoes <- 
    ...
    mutate(
      ...
      sigla = 
        case_when(
          str_detect(tolower(sigla), "ptdob") ~ "PTdoB",
...
fetch_all_orientacoes <- function(votacoes_datapath = here::here("crawler/raw_data/tabela_votacoes.csv")){
  votacoes <- readr::read_csv(votacoes_datapath, col_types = "cicccccccc")
  ...
  orientacoes <- purrr::pmap_dfr(list(votacoes$id_proposicao, 
                                      votacoes$id_sessao, 
                                      votacoes$resumo,
  ...
  fetch_all_orientacoes <- function(votacoes_datapath = here::here("crawler/raw_data/tabela_votacoes.csv")){
    ...
    readr::write_csv(orientacoes, here::here("crawler/raw_data/orientacoes.csv"))
