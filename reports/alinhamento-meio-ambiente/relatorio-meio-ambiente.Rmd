...
orientacoes <- readr::read_csv(here::here("crawler/raw_data/orientacoes.csv"))
...
As barras de Samuel Moreita (PSDB/SP) e de Nelson Pellegrino (PT/BA) são menores pois ambos não participaram de algumas votações no período analisado (legislatura 15-18). Segundo suas biografias na Câmara ([aqui](https://www.camara.leg.br/deputados/178990/biografia
...
Outro ponto a se informar é que os partidos entitulados de Governo, Maioria e Minoria dizem respeito aos blocos parlamentares da 55ª legislatura, que vai de 2015 a 2018.
...
A visualização a seguir apresenta a distribuição do alinhamento dos Deputados nas Comissões. O eixo x indica o alinhamento calculado, quanto maior o pico da "onda" na distribuição, maior é a quantidade de deputados naquela faixa de alinhamento.

```{r fig.height=10}
levels <- deputados_atuais_comissoes %>% 
  mutate(sigla = dplyr::if_else(is.na(sigla), "Nenhuma", sigla)) %>% 
  group_by(sigla) %>% 
  summarise(median = median(alinhamento * 100)) %>% 
  arrange(desc(median)) %>% 
  pull(sigla)

deputados_atuais_comissoes %>% 
  group_by(sigla) %>% 
  mutate(median = median(alinhamento * 100)) %>% 
  ungroup() %>% 
  mutate(sigla = dplyr::if_else(is.na(sigla), "Nenhuma", sigla)) %>% 
  ggplot(aes(x = alinhamento * 100, y = forcats::fct_rev(factor(sigla, levels = levels)), fill = "a")) + 
    geom_density_ridges(
      scale = 1
      # jittered_points = TRUE,
      # position = position_points_jitter(width = 0.05, height = 0),
      # point_shape = '°', point_size = 4, point_alpha = 0.5, alpha = 0.7,
    ) +
  ...
  scale_x_continuous(breaks = seq(0, 100, 10), expand = c(0.01, 0)) +
  theme_ipsum_rc() 
```

O ponto preto na visualização indica a mediana dos alinhamentos dos componentes da Comissão.

Pela visualização é notável que a concentração de deputados com alinhamento ao meio ambiente ocorre em valores abaixo de 50%. A Comissão com a maior mediana com relação a seus integrantes é a CCULT com 42.9% de alinhamento.
