...
pallete <- c(partidos_cores[1], c('#C4C4C4'), partidos_cores[-1])
...
plot_alinhamento <- function(partido, cor, data) {
  ...
  data %>%
    filter(cpf %in% cpfs_candidatos_eleitos, partido_alt == partido) %>% 
    plot_ly() %>%
    add_trace(x = ~mean,
              y = ~id,
              color = ~partido_alt,
...
plots <- tibble(partido = partidos_completo, cor = pallete) %>% 
...
subplot(plots, nrows = 7, shareX = TRUE, shareY = TRUE)

# mdb <- plot_alinhamento(candidatos_ideal_points, "#ff7f00", "MDB")
# outros <- plot_alinhamento(candidatos_ideal_points, "#C4C4C4", "Outros")
# pcdob <- plot_alinhamento(candidatos_ideal_points, "#984ea3", "PC do B")
# psdb <- plot_alinhamento(candidatos_ideal_points, "#004997", "PSDB")
# psl <- plot_alinhamento(candidatos_ideal_points, "#4D9B66", "PSL")
# psol <- plot_alinhamento(candidatos_ideal_points, "#ffde00", "PSOL")
# pt <- plot_alinhamento(candidatos_ideal_points, "#e31a1c", "PT")
```
...
```{r}
candidatos_ideal_points %>%
  filter(cpf %in% cpfs_candidatos_eleitos) %>% 
  plot_ly() %>%
  add_trace(x = ~mean,
            y = ~id,
            color = ~partido_alt,
