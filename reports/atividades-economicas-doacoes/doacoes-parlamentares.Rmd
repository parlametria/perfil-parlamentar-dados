...
plot_geom_count_density <- function(data, title, numero_minimo_parlamentares_por_grupo_economico, casa) {
  ...
  data %>%
    ...
    geom_point(aes(x = x_barra, y = 10.3), size = 3.5, color = "#dc267f", shape = 124) +
...
**Foram desconsideradas 25% das doações com os menores valores dentre o conjunto total de doações.** Com isto, as doações de grupos econômicos a deputados com valor inferior a R$ `r doacoes_sumario_camara %>% pull(total_por_atividade) %>% quantile(0.25) %>% round(2)` foram desconsideradas na visualização abaixo.
```{r}
plot_geom_count_density(doacoes_sumario_alt_camara, "Distribuição da proporção \nde doação para deputados por Grupo", 10, "camara")
```

**Tamém foram desconsideradas 25% das doações com os menores valores dentre o conjunto total de doações no senado.** Com isto, as doações de grupos a senadores com valor inferior a R$ `r doacoes_sumario_senado %>% pull(total_por_atividade) %>% quantile(0.25) %>% round(2)` não foram consideradas no gráfico a seguir.
```{r}
plot_geom_count_density(doacoes_sumario_alt_senado, "Distribuição da proporção \nde doação para senadores por Grupo", 5, "senado")
```
