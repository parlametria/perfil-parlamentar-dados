...
A seguir iremos visualizar quais os setores econômicos que mais doaram para deputados em 2018.
...
A seguir a lista dos setores que mais doaram para senadores
...
Abaixo exibimos a relação entre o valor total recebido em campanhas por setor econômico e o número de parlamentares que receberam essas doações. Todos os setores econômicos foram considerados nas visualizações.
...
É possível perceber a ocorrência de vários casos de deputados com proporção de doação vinda do setor econômico superior a 50% na Câmara. Em alguns casos o valor se aproxima bastante de 100%, ou seja, toda a doação recebida pelo deputado veio de sócios ligados a empresas do setor econômico.
...
lista_partidos_camara <- doacoes_partidos_sumario_camara %>% 
  spread(key, value) %>% 
  filter(proporcao_partido_socios >= media_geral) %>% 
  arrange(desc(proporcao_partido_socios)) %>% 
  pull(sg_partido)
...
lista_partidos_senado <- doacoes_partidos_sumario_senado %>% 
  spread(key, value) %>% 
  filter(proporcao_partido_socios >= media_geral) %>% 
  arrange(desc(proporcao_partido_socios)) %>% 
  pull(sg_partido)
