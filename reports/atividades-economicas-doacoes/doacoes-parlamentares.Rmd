...
**Também foram desconsideradas 25% das doações com os menores valores dentre o conjunto total de doações no senado.** Com isto, as doações de grupos a senadores com valor inferior a R$ `r doacoes_sumario_senado %>% pull(total_por_atividade) %>% quantile(0.25) %>% round(2)` não foram consideradas no gráfico a seguir.
```{r}
