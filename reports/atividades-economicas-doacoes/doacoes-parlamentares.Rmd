...
visao_geral %>%
  ...
  ggplot(aes(x = "origem", y = proporcao, fill = origem)) +
  geom_chicklet(width = .7, position = "stack") +
  geom_text_repel(aes(label = stringr::str_remove(string = format_currency(valor), pattern = " milhões")),
          hjust = -0.05,
          position = "stack"
          ) +
  coord_flip() +
  ...
  labs(x = "",
       y = "Porcentagem do Valor total de financiamento para campanha de deputados",
       title = "Origem do financiamento de campanha \ndos deputados",
...
b <- visao_geral %>%
  ...
  ggplot(aes(x = origem, y = proporcao, fill = origem)) +
  geom_chicklet(width = .7, position = "dodge") +
  geom_text(aes(label = format_currency(valor)),
            hjust = -0.05,
            position = "dodge"
            ) +
  coord_flip() +
