...
coautorias <- get_coautorias(peso_autorias_docs_camara, autorias_camara, parlamentares_camara)
...
Para geração deste relatório foram utilizadas as proposições da plataforma [Leggo](https://leggo.org.br) listadas abaixo. Para cada uma das proposições, existe uma série de outras proposições relacionadas, como emendas, pareceres, requerimentos, etc., que também foram consideradas para a criação dos grupos parlamentares. 
...
choices_ids <- choices_ids$id_principal
...
getCoautorias <- reactive({
  nodes_edges <- 
    generate_nodes_and_edges(
      coautorias %>% 
        filter(id_principal == input$prop_id))
...
getPesosProp <- function(prop_id) {
  coautorias %>% 
    filter(id_principal == prop_id) %>% 
    select(peso_arestas) %>% 
    as.list() %>% 
...
#    updateSelectInput(session,'min_peso',
#            choices=getPesosProp(input$Select1))
# })
...
output$dt <- 
  DT::renderDataTable(
    coautorias %>% 
      select(id_principal, nome.x, nome.y, peso_arestas, num_coautorias) %>% 
      distinct() %>% 
      # filter(peso_arestas >=  input$min_peso) %>% 
      filter(id_principal == input$prop_id) %>%
