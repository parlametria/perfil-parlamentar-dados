...
coautorias <- rbind(coautorias_camara, coautorias_senado) %>% left_join(props, by = "id_principal") %>% 
...
output$dt <- 
  DT::renderDataTable(
    coautorias %>% 
      select(id_principal, nome.x, nome.y, peso_arestas, num_coautorias, nome_formal) %>% 
      distinct() %>% 
      filter(nome_formal == input$nome_formal) %>%
